...
function finishAction() {
    renderGame();
    if (playerHand.length === 0) { gameActive = false; if(isMultiplayer) conn.send({type:'END'}); showEndScreen(true); return; }
    if (opponentHand.length === 0) { gameActive = false; showEndScreen(false); return; }

    // Se non è uno skip o reverse, cambia turno
...
function renderGame() {
    ...
    opponentHand.forEach(() => { oHand.innerHTML += `<div class="card-back-classic" style="margin:0 -22px">MASTER UNO</div>`; });
...
// Funzioni PeerJS e UI (Invariate)
const initPeer = () => {
...
function showToast(m) { let c = document.getElementById('toast-container'); const t = document.createElement('div'); t.className = 'toast'; t.innerText = m; c.appendChild(t); setTimeout(() => t.remove(), 2500); }
function showEndScreen(win) { setTimeout(() => { const s = document.getElementById("endScreen"); const t = document.getElementById("endTitle"); s.classList.remove("hidden"); t.innerText = win ? "HAI VINTO!" : "HAI PERSO!"; t.className = "end-title " + (win ? "win-text" : "lose-text"); if (win) confetti({ particleCount: 150, spread: 70, origin: { y: 0.6 }, zIndex: 6000 }); }, 1500); }
document.getElementById("playBotBtn").onclick = () => { isMultiplayer = false; startG(true); };
document.getElementById("connectBtn").onclick = () => { let id = document.getElementById("friendIdInput").value.trim().toUpperCase(); if (id) { conn = peer.connect(id); isMultiplayer = true; setupChat(); conn.on('open', () => startG(true)); } };
