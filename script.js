...
function playCard(i) {
    ...
    if (isValidMove(card)) {
        ...
        if (drawStack > 0) {
            showToast("DEVI RISPONDERE CON UN " + (topCard.value === "wild4" ? "+4!" : "+2!"));
        }
    }
}
...
function setupChat() {
    conn.on('data', d => {
        ...
        else if (d.type === 'END') showEndScreen(false);
    });
}
...
document.getElementById("playAgainBtn").onclick = () => { if (isMultiplayer) startG(true); else startG(true); };
